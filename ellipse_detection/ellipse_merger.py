...
class EllipseMerger(object):
    ...
    def __init__(
        self, image_w, image_h, identify_threshold=0.1, axis_ratio_threshold=0.9
    ):
    ...
    def _is_same(self, ellipse1, ellipse2):
        ...
        dist[2] = abs(float(ellipse1.major_len - ellipse2.major_len)) / max(
            ellipse1.major_len, ellipse2.major_len
        )  # Distance of a
        dist[3] = abs(float(ellipse1.minor_len - ellipse2.minor_len)) / min(
            ellipse1.minor_len, ellipse2.minor_len
        )  # Distance of b

        # Distance of theta
        r1 = ellipse1.minor_len / ellipse1.major_len
        r2 = ellipse2.minor_len / ellipse2.major_len
        if r1 >= self.AXIS_RATIO_THRESHOLD and r2 >= self.AXIS_RATIO_THRESHOLD:
            dist[4] = 0
        elif r1 >= self.AXIS_RATIO_THRESHOLD and r2 < self.AXIS_RATIO_THRESHOLD:
            dist[4] = 1
        elif r1 < self.AXIS_RATIO_THRESHOLD and r2 >= self.AXIS_RATIO_THRESHOLD:
            dist[4] = 1
        elif r1 < self.AXIS_RATIO_THRESHOLD and r2 < self.AXIS_RATIO_THRESHOLD:
        ...
        return np.all(dist < self.IDENTIFY_THRESHOLD)
    ...
    def merge(self, ellipses):
        ...
        return sorted(merged_ellipses, reverse=True, key=lambda x: x.accuracy_score)
