...
class EllipseCenterEstimator(object):
    ...
    def estimate(self, seg_a, seg_b, debug_image=None):
        ...
        if debug_image is not None:
            midpoints, ra, ma = EllipseCenterEstimator._compute_chord_midpoints(
                seg_a, seg_b, seg_ab_midpoints_image
            )
        else:
            midpoints, ra, ma = EllipseCenterEstimator._compute_chord_midpoints(
                seg_a, seg_b
        ...
        if debug_image is not None:
            ...
            cv2.line(
                seg_ab_midpoints_image,
                tuple(pt1.astype(np.int32)),  # type: ignore
                tuple(pt2.astype(np.int32)),
                (255, 0, 0),
            )

        if debug_image is not None:
            midpoints, rb, mb = EllipseCenterEstimator._compute_chord_midpoints(
                seg_b, seg_a, seg_ba_midpoints_image
            )
        else:
            midpoints, rb, mb = EllipseCenterEstimator._compute_chord_midpoints(
                seg_b, seg_a
        ...
        cab_x = (mb[1] - tb * mb[0] - ma[1] + ta * ma[0]) / (ta - tb)  # type: ignore
        cab_y = (ta * mb[1] - tb * ma[1] + ta * tb * (ma[0] - mb[0])) / (ta - tb)  # type: ignore
        ...
        self._seg_pair_cache[cache_key] = SegmentPair(
            seg_b, seg_a, np.array([cab_x, cab_y]), rb, ra, sb, sa
        )
    ...
    def _compute_chord_midpoints(cls, seg_a, seg_b, debug_image=None):
        ...
        samples_a = seg_a.sample_chord_points(
            sample_from=seg_a_sample_from, chord_normal=pha_normal, interval=3
        )
        samples_b = seg_b.sample_chord_points(
            sample_from=Segment.SAMPLE_FROM_MIDDLE_POINT,
            chord_normal=pha_normal,
            interval=3,
        )
        ...
        if debug_image is not None:
            ...
            cv2.line(
                debug_image,
                tuple(pa.astype(np.int32)),
                tuple(hb.astype(np.int32)),
                (255, 0, 0),
            )
        ...
        for i in range(num_midpoints):
            ...
            cv2.circle(
                debug_image,  # type: ignore
                tuple(midpoints[i].astype(np.int32)),
                2,
                (255, 0, 0),
                -1,
            )
