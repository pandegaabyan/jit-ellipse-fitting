...
class Ellipse(object):
    def __init__(self, center, major_len, minor_len, angle, lying_threshold=0.1):
        """Constructor.
    ...
    def __str__(self):
        return "{{center: {0}, major_len: {1}, minor_len: {2}, angle: {3}, accuracy_score: {4}}}".format(
            self._center,
            self._major_len,
            self._minor_len,
            self._angle,
    ...
    def _is_lying(self, x):
        return (1 - self.LYING_THRESHOLD) <= x <= (1 + self.LYING_THRESHOLD)
    ...
    def draw(self, image):
        cv2.ellipse(
            image,
            tuple(self._center.astype(np.int32)),
            (int(self._major_len), int(self._minor_len)),
