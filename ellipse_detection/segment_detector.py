...
class SegmentDetector(object):
    ...
    def _trace_segment(self, image, image_dir, footprint, segment_id, start):
        ...
        return Segment(
            points,
            segment_id,
            straight_ratio_threshold=self.STRAIGHT_RATIO_THRESHOLD,
        )

    def _decide_convexity(self, footprint, segment, main_class):
    ...
    def detect(self, image, image_edge=None):
        ...
        image_gauss = cv2.GaussianBlur(
            src=image, ksize=self.GAUSSIAN_K_SIZE, sigmaX=self.GAUSSIAN_SIGMA_X
        )
        image_dx = cv2.Sobel(src=image_gauss, ddepth=cv2.CV_32FC1, dx=1, dy=0)  # type: ignore
        image_dy = cv2.Sobel(src=image_gauss, ddepth=cv2.CV_32FC1, dx=0, dy=1)  # type: ignore
        ...
        )  # Edge gradient is upward to the right
        image_24 = np.zeros(
            shape=image.shape, dtype=np.uint8
        )  # Edge gradient is downward to the right
        ...
        footprint = np.zeros(shape=image.shape, dtype=np.int64)
        for i in range(image.shape[0]):
            for j in range(image.shape[1]):
        ...
        for segment in segments_13:
            convexity = self._decide_convexity(
                footprint=footprint,
                segment=segment,
                main_class=SegmentDetector.MAIN_CLASS_13,
            )
        ...
        for segment in segments_24:
            convexity = self._decide_convexity(
                footprint=footprint,
                segment=segment,
                main_class=SegmentDetector.MAIN_CLASS_24,
            )
            if convexity == SegmentDetector.CONVEXITY_UPWARD:
