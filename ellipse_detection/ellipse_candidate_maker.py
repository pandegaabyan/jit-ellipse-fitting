...
class EllipseCandidateMaker(object):
    def __init__(self, ellipse_center_reject_distance=50):
    ...
    def _is_arrangement_valid(cls, seg_i, seg_j, seg_k, ccw_order):
        ...
        if ccw_order == (1, 2, 4):
            if seg_j.is_right(seg_i) and seg_k.is_up(seg_i) and seg_j.is_right(seg_k):
                return True
        elif ccw_order == (2, 3, 1):
            if seg_j.is_up(seg_i) and seg_k.is_left(seg_i) and seg_j.is_right(seg_k):
                return True
        elif ccw_order == (3, 4, 2):
            if seg_j.is_left(seg_i) and seg_k.is_down(seg_i) and seg_j.is_left(seg_k):
                return True
        elif ccw_order == (4, 1, 3):
            if seg_j.is_down(seg_i) and seg_k.is_right(seg_i) and seg_j.is_left(seg_k):
                return True
        ...
        for seg_i in segments[0]:
            for seg_j in segments[1]:
                for seg_k in segments[3]:
                    if EllipseCandidateMaker._is_arrangement_valid(
                        seg_i, seg_j, seg_k, (1, 2, 4)
                    ):
                        # debug
                        ...
                        if debug_image is not None:
                            seg_pair_ij = ellipse_ce.estimate(
                                seg_i, seg_j, seg_pair_ij_image
                            )
                            seg_pair_ki = ellipse_ce.estimate(
                                seg_k, seg_i, seg_pair_ki_image
                            )
                        else:
                        ...
                        if debug_image is not None:
                            cv2.circle(
                                seg_pair_ij_image,
                                tuple(seg_pair_ij.ellipse_center.astype(np.int32)),
                                2,
                                (255, 0, 0),
                                -1,
                            )
                            cv2.circle(
                                seg_pair_ki_image,
                                tuple(seg_pair_ki.ellipse_center.astype(np.int32)),
                                2,
                                (255, 0, 0),
                                -1,
                            )
                        ...
                        if (
                            np.linalg.norm(
                                seg_pair_ij.ellipse_center - seg_pair_ki.ellipse_center
                            )
                            < self.ELLIPSE_CENTER_REJECT_DISTANCE
                        ):
