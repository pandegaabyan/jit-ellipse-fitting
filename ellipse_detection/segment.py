...
class Segment(object):
    ...
    def __init__(self, points, segment_id, straight_ratio_threshold=30):
        """Constructor
        ...
        self._points = points
        self._rot_rect = cv2.minAreaRect(
            points
    ...
    def is_straight(self):
        ...
        if ratio > self.STRAIGHT_RATIO_THRESHOLD:
            return True
    ...
    def draw(self, image, label=None):
        ...
        for i, p in enumerate(self._points):
            cv2.circle(
                image, tuple(p), 1, ((1 - float(i) / n) * 255, 0, float(i) / n * 255)
            )
        ...
        vertices = cv2.boxPoints(self._rot_rect).astype(np.int32)
        begin = vertices[-1]
        for end in vertices:
