...
class EllipseEstimator(object):
    ...
    def _estimate(self, ellipse_cand):
        ...
        (
            ...
            sa_ij,
            sb_ij,
        ) = ellipse_cand.seg_pair_ij.all_params
        ...
        n_acc = [0] * self.NUM_BIN_N_ACCUMULATOR  # N \in [0, 1]
        rho_acc = [0] * self.NUM_BIN_RHO_ACCUMULATOR  # \rho \in [-\pi/2, \pi/2]
        for i in range(4):
                ...
                for q4 in q4_list:
                    ...
                    try:
                        rho_bin = int(
                            (rho + math.pi / 2)
                            / math.pi
                            * (self.NUM_BIN_RHO_ACCUMULATOR - 1)
                        )
                        n_bin = int(n * (self.NUM_BIN_N_ACCUMULATOR - 1))
                    except ValueError:
                        continue
        ...
        n = np.argmax(n_acc) / (self.NUM_BIN_N_ACCUMULATOR - 1.0)
        rho = (
            np.argmax(rho_acc) / (self.NUM_BIN_RHO_ACCUMULATOR - 1.0) * math.pi
            - math.pi / 2
        )  # Ellipse angle
        k = math.tan(rho)

        a_acc = [0] * (self.MAX_MAJOR_SEMI_AXIS_LEN + 1)
        ...
        accuracy_score = (
            ellipse.count_lying_points(seg_i)
            + ellipse.count_lying_points(seg_j)
            + ellipse.count_lying_points(seg_k)
        ) / float(seg_i.points.shape[0] + seg_j.points.shape[0] + seg_k.points.shape[0])
